...
# Načtení snímků portálů do seznamu
portal_frames = []
for i in range(4):
...
frame_index = 0
animation_speed = 200  # Časový interval mezi snímky (v milisekundách)
last_update_time = 0


#Načtení snímků pavouka do seznamu
spider_frames = []
for i in range(2):
...
spider_animation_speed = 500  # Časový interval mezi snímky (v milisekundách)
spider_last_update_time = 0

# Počáteční hodnoty pro pohyb pavouka
spider_y = height // 2  # Počáteční vertikální pozice pavouka
spider_speed = 1  # Rychlost pohybu pavouka

#Pavouci kolize
spider_collision_rect = pygame.Rect(width - spider_frames[frame_index_spider].get_width() - 10 - 10, spider_y - 10, spider_frames[frame_index_spider].get_width() + 10, spider_frames[frame_index_spider].get_height() - 10)
...
def check_collision(rect1, rect2):
    """Funkce pro kontrolu kolize dvou obdélníků."""
